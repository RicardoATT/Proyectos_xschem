v {xschem version=3.4.5 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
B 2 -2030 -1020 -1230 -620 {flags=graph
y1=-0.9
y2=0.9
ypos1=0
ypos2=2

subdivy=1
unity=1
x1=0
x2=1e-05

subdivx=1
xlabmag=1.0
ylabmag=1.0
node="vout

vin
voutori
voutori_new"
color="4 6 10 9"
dataset=-1
unitx=1
logx=0
logy=0

divy=12
divx=20
linewidth_mult=1.6}
B 2 -2040 -470 -1240 -70 {flags=graph
y1=-0.9
y2=0.9
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=0
x2=1e-05
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0
node="vin
vout2"
color="7 9"
dataset=-1
unitx=1
logx=0
logy=0
}
B 2 -800 -410 0 -10 {flags=graph
y1=-0.9
y2=0.9
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=0
x2=1e-05
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0
node="vout3
vin"
color="6 7"
dataset=-1
unitx=1
logx=0
logy=0
}
B 2 -2020 -20 -1220 380 {flags=graph
y1=-0.9
y2=0.9
ypos1=0
ypos2=2

subdivy=1
unity=1
x1=0
x2=1e-05
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0
node="vin

voutori_300k"
color="9 10"
dataset=-1
unitx=1
logx=0
logy=0
divy=20}
N -1140 -750 -1080 -750 {
lab=vin}
N -1080 -750 -1080 -730 {
lab=vin}
N -590 -960 -590 -950 {
lab=VDD}
N -980 -820 -980 -800 {
lab=VDD}
N -1080 -710 -1080 -670 {
lab=GND}
N -980 -700 -980 -670 {
lab=#net1}
N -1030 -670 -1030 -660 {
lab=GND}
N -1140 -690 -1140 -680 {
lab=GND}
N -590 -890 -590 -880 {
lab=GND}
N -1020 -830 -1020 -820 {
lab=VDD}
N -1030 -890 -1020 -890 {
lab=VDD}
N -900 -750 -900 -730 {
lab=vout}
N -1080 -940 -1080 -770 {
lab=vout}
N -900 -940 -900 -750 {
lab=vout}
N -1020 -890 -1020 -830 {
lab=VDD}
N -1080 -670 -1030 -670 {
lab=GND}
N -980 -670 -980 -660 {
lab=#net1}
N -980 -600 -980 -590 {
lab=GND}
N -980 -410 -980 -390 {
lab=VDD}
N -1080 -300 -1080 -260 {
lab=GND}
N -980 -290 -980 -260 {
lab=#net2}
N -1030 -260 -1030 -250 {
lab=GND}
N -1020 -420 -1020 -410 {
lab=VDD}
N -1030 -480 -1020 -480 {
lab=VDD}
N -1020 -480 -1020 -420 {
lab=VDD}
N -1080 -260 -1030 -260 {
lab=GND}
N -980 -260 -980 -250 {
lab=#net2}
N -980 -190 -980 -180 {
lab=GND}
N -1210 -340 -1180 -340 {
lab=vin}
N -1120 -340 -1080 -340 {
lab=#net3}
N -1080 -360 -1080 -340 {
lab=#net3}
N -1110 -440 -1110 -340 {
lab=#net3}
N -900 -440 -900 -340 {
lab=vout2}
N -1050 -530 -1050 -440 {
lab=vout2}
N -1050 -530 -900 -530 {
lab=vout2}
N -900 -530 -900 -440 {
lab=vout2}
N -900 -340 -900 -330 {
lab=vout2}
N -1080 -320 -1080 -300 {
lab=GND}
N -290 -810 -290 -790 {
lab=VDD}
N -390 -700 -390 -660 {
lab=GND}
N -290 -690 -290 -660 {
lab=#net4}
N -340 -660 -340 -650 {
lab=GND}
N -330 -820 -330 -810 {
lab=VDD}
N -340 -880 -330 -880 {
lab=VDD}
N -330 -880 -330 -820 {
lab=VDD}
N -390 -660 -340 -660 {
lab=GND}
N -290 -660 -290 -650 {
lab=#net4}
N -290 -590 -290 -580 {
lab=GND}
N -520 -740 -490 -740 {
lab=vin}
N -430 -740 -390 -740 {
lab=#net5}
N -390 -760 -390 -740 {
lab=#net5}
N -420 -840 -420 -740 {
lab=#net5}
N -210 -840 -210 -740 {
lab=vout3}
N -360 -930 -360 -840 {
lab=vout3}
N -360 -930 -210 -930 {
lab=vout3}
N -210 -930 -210 -840 {
lab=vout3}
N -210 -740 -210 -730 {
lab=vout3}
N -390 -720 -390 -700 {
lab=GND}
N -430 -610 -430 -530 {
lab=#net5}
N -430 -710 -430 -610 {
lab=#net5}
N -430 -710 -410 -710 {
lab=#net5}
N -410 -740 -410 -710 {
lab=#net5}
N -570 -600 -490 -600 {
lab=#net6}
N -490 -610 -490 -600 {
lab=#net6}
N -530 -470 -530 -450 {
lab=GND}
N -570 -470 -530 -470 {
lab=GND}
N -570 -540 -570 -470 {
lab=GND}
N -530 -530 -490 -530 {
lab=#net7}
N -2370 -790 -2310 -790 {
lab=vin}
N -2310 -790 -2310 -770 {
lab=vin}
N -2210 -860 -2210 -840 {
lab=VDD}
N -2310 -750 -2310 -710 {
lab=GND}
N -2210 -740 -2210 -710 {
lab=#net8}
N -2260 -710 -2260 -700 {
lab=GND}
N -2250 -870 -2250 -860 {
lab=VDD}
N -2260 -930 -2250 -930 {
lab=VDD}
N -2130 -790 -2130 -770 {
lab=voutori}
N -2310 -980 -2310 -810 {
lab=voutori}
N -2310 -980 -2130 -980 {
lab=voutori}
N -2130 -980 -2130 -790 {
lab=voutori}
N -2250 -930 -2250 -870 {
lab=VDD}
N -2310 -710 -2260 -710 {
lab=GND}
N -2210 -710 -2210 -700 {
lab=#net8}
N -2210 -640 -2210 -630 {
lab=GND}
N -2390 -380 -2330 -380 {
lab=vin}
N -2330 -380 -2330 -360 {
lab=vin}
N -2230 -450 -2230 -430 {
lab=VDD}
N -2330 -340 -2330 -300 {
lab=GND}
N -2230 -330 -2230 -300 {
lab=#net9}
N -2280 -300 -2280 -290 {
lab=GND}
N -2270 -460 -2270 -450 {
lab=VDD}
N -2280 -520 -2270 -520 {
lab=VDD}
N -2150 -380 -2150 -360 {
lab=voutori_new}
N -2330 -570 -2330 -400 {
lab=voutori_new}
N -2330 -570 -2150 -570 {
lab=voutori_new}
N -2150 -570 -2150 -380 {
lab=voutori_new}
N -2270 -520 -2270 -460 {
lab=VDD}
N -2330 -300 -2280 -300 {
lab=GND}
N -2230 -300 -2230 -290 {
lab=#net9}
N -2230 -230 -2230 -220 {
lab=GND}
N -1080 -940 -900 -940 {
lab=vout}
N -2380 10 -2320 10 {
lab=vin}
N -2320 10 -2320 30 {
lab=vin}
N -2220 -60 -2220 -40 {
lab=VDD}
N -2320 50 -2320 90 {
lab=GND}
N -2220 60 -2220 90 {
lab=#net10}
N -2270 90 -2270 100 {
lab=GND}
N -2260 -70 -2260 -60 {
lab=VDD}
N -2270 -130 -2260 -130 {
lab=VDD}
N -2140 10 -2140 30 {
lab=voutori_100k}
N -2320 -180 -2320 -10 {
lab=voutori_100k}
N -2320 -180 -2140 -180 {
lab=voutori_100k}
N -2140 -180 -2140 10 {
lab=voutori_100k}
N -2260 -130 -2260 -70 {
lab=VDD}
N -2320 90 -2270 90 {
lab=GND}
N -2220 90 -2220 100 {
lab=#net10}
N -2220 160 -2220 170 {
lab=GND}
N -1200 110 -1140 110 {
lab=vin}
N -1140 110 -1140 130 {
lab=vin}
N -1040 40 -1040 60 {
lab=VDD}
N -1140 150 -1140 190 {
lab=GND}
N -1040 160 -1040 190 {
lab=#net11}
N -1090 190 -1090 200 {
lab=GND}
N -1080 30 -1080 40 {
lab=VDD}
N -1090 -30 -1080 -30 {
lab=VDD}
N -960 110 -960 130 {
lab=voutori_300k}
N -1140 -80 -1140 90 {
lab=voutori_300k}
N -1140 -80 -960 -80 {
lab=voutori_300k}
N -960 -80 -960 110 {
lab=voutori_300k}
N -1080 -30 -1080 30 {
lab=VDD}
N -1140 190 -1090 190 {
lab=GND}
N -1040 190 -1040 200 {
lab=#net11}
N -1040 260 -1040 270 {
lab=GND}
C {/home/ricardoatt/RATT_repos/Proyectos_xschem/two_stage_opamp/two_stage_opamp.sym} -870 -860 0 0 {name=x1 W1=2.13 W2=9 W3=8.7 W4=9 W5=9.8 W6=8.7 L1=0.3965 L2=0.3965 L3=0.3965 L4=0.9 L5=1.2 L6=0.8}
C {devices/vdd.sym} -980 -820 0 0 {name=l1 lab=VDD}
C {devices/vdd.sym} -590 -960 0 0 {name=l2 lab=VDD}
C {devices/vsource.sym} -590 -920 0 0 {name=V1 value=\{v1vdd\} savecurrent=false}
C {devices/vsource.sym} -1140 -720 0 0 {name=V2 value="SINE(0 0.9 100k)" savecurrent=false}
C {devices/gnd.sym} -1030 -660 0 0 {name=l3 lab=GND}
C {devices/gnd.sym} -1140 -680 0 0 {name=l4 lab=GND}
C {devices/gnd.sym} -590 -880 0 0 {name=l5 lab=GND}
C {devices/vdd.sym} -1030 -890 0 0 {name=l7 lab=VDD}
C {devices/code_shown.sym} -840 -900 0 0 {name=s1 only_toplevel=false value="
.tran 0.005u 10u 0
.param v1vdd=0.9
.param v3vss=-0.9
.control
	run
	
	plot v(vout) 
	plot v(vout2) 
	plot v(vin) 
	write sine_prueba.raw
.endc
.save all
"}
C {devices/lab_pin.sym} -900 -740 0 1 {name=p1 sig_type=std_logic lab=vout}
C {sky130_fd_pr/corner.sym} -850 -660 0 0 {name=CORNER only_toplevel=true corner=tt}
C {devices/launcher.sym} -1100 -570 0 0 {name=h5
descr="load waves" 
tclcommand="xschem raw_read $netlist_dir/sine_prueba.raw"
}
C {devices/lab_pin.sym} -1110 -750 1 0 {name=p2 sig_type=std_logic lab=vin}
C {devices/vsource.sym} -980 -630 0 0 {name=V3 value=\{v3vss\} savecurrent=false}
C {devices/gnd.sym} -980 -590 0 0 {name=l6 lab=GND}
C {/home/ricardoatt/RATT_repos/Proyectos_xschem/two_stage_opamp/two_stage_opamp.sym} -870 -450 0 0 {name=x2 W1=2.13 W2=9 W3=8.7 W4=9 W5=9.8 W6=8.7 L1=0.3965 L2=0.3965 L3=0.3965 L4=0.9 L5=1.2 L6=0.8}
C {devices/vdd.sym} -980 -410 0 0 {name=l8 lab=VDD}
C {devices/gnd.sym} -1030 -250 0 0 {name=l10 lab=GND}
C {devices/vdd.sym} -1030 -480 0 0 {name=l13 lab=VDD}
C {devices/lab_pin.sym} -900 -330 0 1 {name=p3 sig_type=std_logic lab=vout2}
C {devices/lab_pin.sym} -1210 -340 1 0 {name=p4 sig_type=std_logic lab=vin}
C {devices/vsource.sym} -980 -220 0 0 {name=V6 value=\{v3vss\} savecurrent=false}
C {devices/gnd.sym} -980 -180 0 0 {name=l14 lab=GND}
C {devices/res.sym} -1150 -340 1 0 {name=R1
value=100k
footprint=1206
device=resistor
m=1}
C {devices/res.sym} -1080 -440 1 0 {name=R2
value=100k
footprint=1206
device=resistor
m=1}
C {/home/ricardoatt/RATT_repos/Proyectos_xschem/two_stage_opamp/two_stage_opamp.sym} -180 -850 0 0 {name=x3 W1=2.13 W2=9 W3=8.7 W4=9 W5=9.8 W6=8.7 L1=0.3965 L2=0.3965 L3=0.3965 L4=0.9 L5=1.2 L6=0.8}
C {devices/vdd.sym} -290 -810 0 0 {name=l9 lab=VDD}
C {devices/gnd.sym} -340 -650 0 0 {name=l11 lab=GND}
C {devices/vdd.sym} -340 -880 0 0 {name=l12 lab=VDD}
C {devices/lab_pin.sym} -210 -730 0 1 {name=p5 sig_type=std_logic lab=vout3}
C {devices/lab_pin.sym} -520 -740 1 0 {name=p6 sig_type=std_logic lab=vin}
C {devices/vsource.sym} -290 -620 0 0 {name=V4 value=\{v3vss\} savecurrent=false}
C {devices/gnd.sym} -290 -580 0 0 {name=l15 lab=GND}
C {devices/res.sym} -460 -740 1 0 {name=R3
value=100k
footprint=1206
device=resistor
m=1}
C {devices/res.sym} -390 -840 1 0 {name=R4
value=100k
footprint=1206
device=resistor
m=1}
C {devices/res.sym} -460 -610 1 0 {name=R5
value=100k
footprint=1206
device=resistor
m=1}
C {devices/res.sym} -460 -530 1 0 {name=R6
value=100k
footprint=1206
device=resistor
m=1}
C {devices/vsource.sym} -570 -570 0 0 {name=V5 value=0.1 savecurrent=false}
C {devices/vsource.sym} -530 -500 0 0 {name=V7 value=-0.2 savecurrent=false}
C {devices/gnd.sym} -530 -450 0 0 {name=l16 lab=GND}
C {/home/ricardoatt/RATT_repos/Proyectos_xschem/two_stage_opamp/two_stage_opamp.sym} -2100 -900 0 0 {name=x4 W1=5 W2=5 W3=3 W4=4.5 W5=4 W6=5 L1=0.15 L2=0.15 L3=0.15 L4=0.15 L5=3 L6=0.15}
C {devices/vdd.sym} -2210 -860 0 0 {name=l17 lab=VDD}
C {devices/gnd.sym} -2260 -700 0 0 {name=l19 lab=GND}
C {devices/vdd.sym} -2260 -930 0 0 {name=l22 lab=VDD}
C {devices/lab_pin.sym} -2130 -780 0 1 {name=p7 sig_type=std_logic lab=voutori}
C {devices/lab_pin.sym} -2340 -790 1 0 {name=p8 sig_type=std_logic lab=vin}
C {devices/vsource.sym} -2210 -670 0 0 {name=V10 value=\{v3vss\} savecurrent=false}
C {devices/gnd.sym} -2210 -630 0 0 {name=l23 lab=GND}
C {/home/ricardoatt/RATT_repos/Proyectos_xschem/two_stage_opamp/two_stage_opamp.sym} -2120 -490 0 0 {name=x5 W1=2.129 W2=9.0 W3=8.7 W4=9.0 W5=9.8 W6=8.7 L1=0.397 L2=0.397 L3=0.397 L4=0.9 L5=1.2 L6=0.8}
C {devices/vdd.sym} -2230 -450 0 0 {name=l18 lab=VDD}
C {devices/gnd.sym} -2280 -290 0 0 {name=l20 lab=GND}
C {devices/vdd.sym} -2280 -520 0 0 {name=l21 lab=VDD}
C {devices/lab_pin.sym} -2150 -370 0 1 {name=p9 sig_type=std_logic lab=voutori_new}
C {devices/lab_pin.sym} -2360 -380 1 0 {name=p10 sig_type=std_logic lab=vin}
C {devices/vsource.sym} -2230 -260 0 0 {name=V8 value=\{v3vss\} savecurrent=false}
C {devices/gnd.sym} -2230 -220 0 0 {name=l24 lab=GND}
C {/home/ricardoatt/RATT_repos/Proyectos_xschem/two_stage_opamp/two_stage_opamp.sym} -2110 -100 0 0 {name=x6 W1=1.875 W2=2.02 W3=2.265 W4=2.02 W5=2.265 W6=9.3 L1=1.623 L2=1.382 L3=0.834 L4=1.382 L5=3.8 L6=1.623}
C {devices/vdd.sym} -2220 -60 0 0 {name=l25 lab=VDD}
C {devices/gnd.sym} -2270 100 0 0 {name=l26 lab=GND}
C {devices/vdd.sym} -2270 -130 0 0 {name=l27 lab=VDD}
C {devices/lab_pin.sym} -2140 20 0 1 {name=p11 sig_type=std_logic lab=voutori_100k}
C {devices/lab_pin.sym} -2350 10 1 0 {name=p12 sig_type=std_logic lab=vin}
C {devices/vsource.sym} -2220 130 0 0 {name=V9 value=\{v3vss\} savecurrent=false}
C {devices/gnd.sym} -2220 170 0 0 {name=l28 lab=GND}
C {/home/ricardoatt/RATT_repos/Proyectos_xschem/two_stage_opamp/two_stage_opamp.sym} -930 0 0 0 {name=x7 W1=2.976 W2=2.402 W3=0.838 W4=2.976 W5=2.402 W6=0.838 L1=1.408 L2=0.754 L3=0.978 L4=1.408 L5=0.754 L6=0.978}
C {devices/vdd.sym} -1040 40 0 0 {name=l29 lab=VDD}
C {devices/gnd.sym} -1090 200 0 0 {name=l30 lab=GND}
C {devices/vdd.sym} -1090 -30 0 0 {name=l31 lab=VDD}
C {devices/lab_pin.sym} -960 120 0 1 {name=p13 sig_type=std_logic lab=voutori_300k}
C {devices/lab_pin.sym} -1170 110 1 0 {name=p14 sig_type=std_logic lab=vin}
C {devices/vsource.sym} -1040 230 0 0 {name=V11 value=\{v3vss\} savecurrent=false}
C {devices/gnd.sym} -1040 270 0 0 {name=l32 lab=GND}
