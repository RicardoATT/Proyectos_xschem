...
def graficar_Vgs_Id(ax, datos, region):
    ...
    for i in range(0, num_cols, 2):
        ...
        if region == " ":
            print("K'l{} = {}".format((i//2)+1, theta[1]/(0.1*W_L[i//2])))
        else:
            print("Vth{} = {} V\nK's{} = {}".format((i//2)+1, round((-theta[0])/theta[1], 4), (i//2)+1, 2*(theta[1]**2)/W_L[i//2]))
    ax.set_title('Curva NFET - Vgs vs' + region + 'Id (Variando W/L)')
    ax.set_xlabel('Vgs (V)')
    if region == " ":
        ax.set_ylabel('Id (A)')
    else:
...
def graficar_Vgs_sqrtId(ax, datos):
    ...
    ax.set_title('Curva NFET - Vgs vs Raíz de Id (Variando Vsb)')
    ax.set_xlabel('Vgs (V)')
    ax.set_ylabel('Raíz de Id (A)')
    ax.grid()
    ax.legend()
...
def graficar_fVsb_Vth(ax, datos):
    ...
    ax.set_title('Curva NFET - función de Vsb vs Vth')
...
def graficar_Vds_Id(ax, datos):
    ...
    ax.set_title('Curva NFET - Vds vs Id')
    ax.set_xlabel('Vds (V)')
    ax.set_ylabel('Id (A)')
    ax.grid()
    ax.legend()
...
archivos = ['NFET_vgs_sqrtid.ssv', 'NFET_vgs_id.ssv', 'NFET_vgs_sqrtid2.ssv', 'NFET_vds_id.ssv']
